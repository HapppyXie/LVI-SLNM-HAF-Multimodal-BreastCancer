...
__all__ = [
    'ResNet', 'resnet10_3D', 'resnet18_3D', 'resnet34_3D', 'resnet50_3D', 'resnet101_3D',
...
def downsample_basic_block(x, planes, stride):
    ...
    参数:
        ...
        planes: 目标输出通道数
        stride: 池化步长，决定下采样倍数
        no_cuda: 是否禁用CUDA（GPU加速）
    
    返回:
        Variable: 下采样后的特征图，通道数扩展到planes
    """
    # 1x1x1平均池化，如果stride=1，不改变特征图大小。如果stride>=2，则下采样特征图大小
    out = F.avg_pool3d(x, kernel_size=1, stride=stride)  # float16
    zero_pads_shape = (out.size(0), planes - out.size(1), out.size(2), out.size(3), out.size(4))
...
class ResNet(nn.Module):
    '''
        实现ResNet 3D模型，支持不同深度（10、18、34、50、101、152、200），深度代表模型可学习参数的层的数量
    '''
    def __init__(self,
                 block,   # 残差块类(BasicBlock 或 Bottleneck)
                 layers,  # 每个stage中残差块的数量
                 input_channels,    # 输入数据的通道数
                 input_size,   # 输入数据的深度 高度 宽度
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            if shortcut_type == 'A':
                ...
                downsample = partial( 
                    ...
                    planes=planes * block.expansion, # 输出通道数
                    stride=stride) # 步长
            else:
                ...
                downsample = nn.Sequential(
                    nn.Conv3d(
                        ...
                        stride=stride, # 步长
                        bias=False),   # 偏置项
                    nn.BatchNorm3d(planes * block.expansion)) # 批归一化层
...
if __name__ == '__main__':
    ...
    input_size=(144, 320, 320)
    # net = resnet18_3D(input_channels=2,input_size=input_size).cuda()
    # net = resnet34_3D(input_channels=2,input_size=input_size).cuda()
    net = resnet50_3D(input_channels=2,input_size=input_size).cuda()
    a = torch.rand(2, 2, input_size[0], input_size[1], input_size[2]).cuda()
    b = net(a)
    print(b)
    print(b.shape)
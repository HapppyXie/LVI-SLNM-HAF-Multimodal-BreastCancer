...
def NiiDataWrite2D(save_path, volumn, spacing, origin, direction, as_type=np.float32):
    '''
    保存二维NIfTI数据
    '''
    spacing = spacing.astype(np.float64)
    raw = sitk.GetImageFromArray(volumn[:, :].astype(as_type))
    spacing_ = (spacing[1], spacing[0])
    raw.SetSpacing(spacing_)
    ...
    if len(origin) > 2:
        ...
        origin_2d = origin
    raw.SetOrigin(origin_2d)
    
    # 设置direction：从3×3转为2×2
    if len(direction) == 9:  # 3×3矩阵
        direction_2d = (direction[0], direction[1], direction[3], direction[4])
    else:
        direction_2d = direction
    raw.SetDirection(direction_2d)
